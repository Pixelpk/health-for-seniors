      ...
      <Sidebar aria-label='Sidebar with multi-level dropdown example' collapsed={isSidebarOpenOnSmallScreens && !isSmallScreen()}>
        <div className='flex h-full flex-col justify-between py-2'>
          <div>
            <Sidebar.Items>
              <Sidebar.ItemGroup>
                <Sidebar.Collapse icon={FaCalendarDays} label='Calender' open={isEcommerceOpen}></Sidebar.Collapse>
                <Sidebar.Item href='/contacts' icon={BiSolidContact} className={'/contacts' === currentPage ? 'bg-gray-100 dark:bg-gray-700' : ''}>
                  Contacts
                </Sidebar.Item>
                <Sidebar.Collapse icon={FaUserGroup} label='Customers' open={isEcommerceOpen}></Sidebar.Collapse>
                <Sidebar.Item icon={BiSolidHome}>Dashboard</Sidebar.Item>
                <Sidebar.Collapse icon={FaInbox} label='Emails' open={isEcommerceOpen}>
                  <Sidebar.Item
                    href='/mailing/compose'
                    icon={FaEdit}
                    className={'/mailing/compose' === currentPage ? 'bg-gray-100 dark:bg-gray-700' : ''}
                  >
                    Compose
                  </Sidebar.Item>
                </Sidebar.Collapse>
